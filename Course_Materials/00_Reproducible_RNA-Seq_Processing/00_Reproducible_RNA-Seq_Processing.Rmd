...
Open R-studio and open the `00_Reproducible_RNA-Seq_Processing.Rmd` file that is located at the course folder. This is an R-notebook file, that has all the code used to generate this document. But, now when you open this file you will be able to run `chunks` of code interactively inside R-studio. Run the following chunk by clicking at the green play button.
...
As we already wrote a `snakemake` pipeline for you, we are going to demonstrate you some of its properties and how to use it.
